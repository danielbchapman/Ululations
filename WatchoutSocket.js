...
export const STATE_CONSTRUCT = 'WATCHOUT_CONSTRUCT'
export const STATE_PRIMARY_AUTHENTICATE = 'WATCHOUT_PRIMARY_AUTHENTICATE'
export const STATE_SEND_CUE = 'WATCHOUT_SEND_CUE'

class WatchoutSocket {
  constructor(address, port, level = 'verbose') {
    this.production = productionAddress
    this.display = displayClusterAddress
    this.debug = debug || false
    this.state = STATE_CONSTRUCT
  }

  conditional(label, first, second) {
    const primary = new net.Socket()
    const secondary = new net.Socket()

    primary.setTimeout(1000) //short life socket
    secondary.setTimeout(1000)
    socket.on('data', data=>{
      info(data)
